...
# Read in data
train = pd.read_csv('data/train.csv', low_memory=False, index_col='id')
test = pd.read_csv('data/test.csv', low_memory=False, index_col='id')
res = pd.read_csv('data/resources.csv', low_memory=False, index_col='id')
...
num_feature = ['teacher_number_of_previously_posted_projects', 'price_sum', 'quantity_sum', 'price_mean', 'quantity_mean', 'price_median', 'quantity_median' ]
...
SS = StandardScaler()
df_scale = SS.fit_transform(df[num_feature])
trn_num = df_scale[:182080]
tst_num = df_scale[182080:]
...
max_words = 2000 #more words for more accuracy
...
len_num = trn_num.shape[1]
len_text = trn_text.shape[1]

size_embedding = 5000


# Create Multi-channel NN Architecture
# categorical channel
inputs1 = Input(shape=(len_cat,))
dense_cat_1 = Dense(256, activation='relu')(inputs1)
dense_cat_1 = BatchNormalization()(dense_cat_1)
dense_cat_1 = Dropout(0.2)(dense_cat_1)
dense_cat_2 = Dense(128, activation='relu')(dense_cat_1)
dense_cat_2 = BatchNormalization()(dense_cat_2)
dense_cat_2 = Dropout(0.2)(dense_cat_2)
dense_cat_3 = Dense(64, activation='relu')(dense_cat_2)
dense_cat_3 = BatchNormalization()(dense_cat_3)
dense_cat_3 = Dropout(0.2)(dense_cat_3)
dense_cat_4 = Dense(32, activation='relu')(dense_cat_3)
dense_cat_4 = BatchNormalization()(dense_cat_4)
dense_cat_4 = Dropout(0.2)(dense_cat_4)
...
dense_cat_4 = BatchNormalization()(dense_cat_4)
dense_cat_4 = Dropout(0.2)(dense_cat_4)
flat2 = Dense(32, activation='relu')(dense_num_4)
...
conv3 = Conv1D(filters=32, kernel_size=8, activation='relu')(pool3)
drop3 = Dropout(0.1)(conv3)
pool3 = MaxPooling1D(pool_size=2)(drop3)
flat3 = Flatten()(pool3)
...
batch_size=128
model.fit([trn_cat, trn_num, trn_text], y, batch_size=batch_size, epochs=3, validation_split=0.2)

submit = model.predict([tst_cat, tst_num, tst_text], batch_size=batch_size, verbose=1)
submission = pd.read_csv('data/sample_submission.csv')
submission['project_is_approved'] = submit
submission.to_csv('submissions/mi_nn.csv', index=False)


# trn_all = np.hstack((trn_cat, trn_num, trn_text))
# tst_all = np.hstack((tst_cat, tst_num, tst_text))
# trn_all.shape
#
# model2 = Sequential()
...
#               metrics=['accuracy', auc])
# batch_size = 2000
# model2.fit(trn_all, y, batch_size=batch_size, epochs=15, validation_split=0.2)
#
# submit = model2.predict(tst_all, batch_size=batch_size,verbose=1)
# print(submit.shape, submit)
# submission = pd.read_csv('data/sample_submission.csv')
# submission['project_is_approved'] = submit
# submission.to_csv('submissions/mi_nn2.csv', index=False)
